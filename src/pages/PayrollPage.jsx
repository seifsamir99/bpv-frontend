...
import { HiCurrencyDollar, HiSave, HiRefresh, HiExclamationCircle } from 'react-icons/hi';
...
export default function PayrollPage() {
  ...
  const handleCalculate = () => {
    ...
    setSelectedIds(new Set(results.map(r => r.employeeId)));
    setOverrides({});
    setSaveSuccess(null);
    setSaveError(null);
  };
  ...
  const handleSave = async () => {
    ...
    const dataToSave = displayData.filter(row => selectedIds.has(row.employeeId));
  ...
  const totals = useMemo(() => {
    ...
    if (selectedData.length === 0) return { count: 0, paidDays: 0, deductionAmount: 0, salaryBeforeOT: 0, otPay: 0, netSalary: 0 };
    ...
    }), { count: 0, paidDays: 0, deductionAmount: 0, salaryBeforeOT: 0, otPay: 0, netSalary: 0 });
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        {displayData.length > 0 && totals && (
          <div className={`grid gap-4 mb-6 ${selectedType === 'labour' ? 'grid-cols-2 sm:grid-cols-5' : 'grid-cols-2 sm:grid-cols-4'}`}>
                ...
                <tbody className="divide-y divide-slate-100">
                  {displayData.map((row, i) => {
                    ...
                    return (
                      <tr
                        key={row.employeeId}
                        className={`hover:bg-slate-50 transition-colors ${!isSelected ? 'opacity-40 bg-slate-50' : ''}`}
                      >
